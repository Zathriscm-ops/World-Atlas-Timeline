...
function check(id, action) {
  try { action(); checks.push({ id, result: 'PASS' }); }
  catch (error) { checks.push({ id, result: 'FAIL', error: error.message }); }
}
const hash = (file) => crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex').toUpperCase();
check('M001-preserved-evidence', () => {
  assert.equal(hash('GATED_DEVELOPMENT_MASTER_PLAN.md'), '6EB5D81750072AC32FA098ED5A66D73EFD4357CABF8C0561E39168FD551207B4');
  assert.equal(hash('docs/archive/MASTER_PROJECT_PLAN_2026-09-14.md'), '7E180668416600DE8CC2D63DDAA8E35EB8C68A8BDE8FBFCF1A64B9A02C490E5B');
  for (let section = 1; section <= 6; section++) assert.ok(read('PRODUCT_VISION.md').includes('## 1.0' + section));
  assert.equal([...read('MVP_SCOPE.md').matchAll(/^\| [RDX]\d+ \| .+ \| (REQUIRED|DEFERRED|REJECTED) \|/gm)].length, 88);
});
if (through >= 2) check('M002-thirty-classifications', () => {
...
if (through >= 3) check('M003-chronology-contract', () => {
  const data = json('data/fixtures/planning/temporal.json');
  const format = (year) => year <= 0 ? String(1 - year) + ' BCE' : String(year) + ' CE';
  const parse = (value) => { const match = /^([1-9]\d*) (BCE|CE)$/.exec(value); assert.ok(match, 'invalid era input'); return match[2] === 'BCE' ? 1 - Number(match[1]) : Number(match[1]); };
  for (const item of data.year_cases) { assert.equal(parse(item.display), item.year); assert.equal(format(item.year), item.display); }
  ...
  const meaning = (e) => Object.fromEntries(Object.entries(e).filter(([key]) => !['display', 'original'].includes(key)));
  const pair = (a, b) => { assert.equal(a <= 0, b <= 0); return String(a <= 0 ? 1-a : a) + '–' + String(b <= 0 ? 1-b : b) + (a <= 0 ? ' BCE' : ' CE'); };
  ...
  const parseExpression = (text) => {
    ...
    if (match) return {kind:'approximate',year:parse(match[1]),precision:'year',bounds:null};
    match = /^(\d+)–(\d+) (BCE|CE)( \(duration\))?$/.exec(text);
    if (match) {
      const a = parse(match[1] + ' ' + match[3]), b = parse(match[2] + ' ' + match[3]);
      return match[4] ? {kind:'duration',start:a,end:b+1,precision:'year'} : {kind:'occurrence_window',earliest:a,latest:b,precision:'year'};
    }
    match = /^(\d+)th century (BCE|CE)$/.exec(text);
    if (match) {
      const n=Number(match[1]), era=match[2];
      return {kind:'century',number:n,era,start:era==='BCE'?1-n*100:(n-1)*100+1,end:era==='BCE'?1-(n-1)*100:n*100+1};
    }
    match = /^(early|mid|late) (.+ century (BCE|CE))$/.exec(text);
    if (match) return {kind:'qualified_period',period:match[2],qualifier:match[1],bounds:null};
...
if (through >= 4) check('M004-uncertainty-semantics', () => {
  const data = json('data/fixtures/planning/uncertainty.json');
  const validate = (item) => {
    assert.ok(['HIGH','MEDIUM','LOW','UNASSESSED'].includes(item.evidence));
    assert.ok(['EXACT_AT_PRECISION','APPROXIMATE','BOUNDED','UNKNOWN'].includes(item.temporal));
    assert.ok(item.rationale.length >= 15 && item.interpretation);
    if (['people','ethnic_group','archaeological_culture','civilization','religion','language'].includes(item.entity_type)) {
      assert.notEqual(item.role, 'control'); assert.notEqual(item.style, 'hard-border');
    }
    if (item.spatial === 'UNKNOWN') assert.equal(item.style, 'unmapped');
    if (item.numeric_probability !== null) assert.ok(item.probability_model && item.calibration_source);
  };
  assert.equal(data.cases.length, 12);
  for (const item of data.cases) item.expected_valid ? validate(item) : assert.throws(() => validate(item), item.id);
...
if (through >= 5) check('M005-fact-provenance', () => {
  ...
  for (const mutation of [
    (d) => { d.claims[0].citation_ids = []; },
    (d) => { d.citations[0].source_id = 'missing'; },
    (d) => { d.sources[0].reliability = 'AI_GENERATED'; },
    (d) => { d.citations[6].consultation = 'DIRECT'; },
    (d) => { d.citations[0].locator = ''; },
  ]) { const invalid = structuredClone(data); mutation(invalid); assert.throws(() => validate(invalid)); }
  const manifests = json('data/research/snapshot-manifest.json');
  assert.deepEqual(manifests.map((m) => m.year), [100, 200]);
...
if (through >= 6) check('M006-fail-closed-rights', () => {
  ...
  Object.assign(valid, {classification:'CC-BY',lineage_review:'CLEARED',rights_review:'CLEARED',historical_review:'APPROVED',content_hash:'a'.repeat(64),derivation_review:'CLEARED'});
  for (const c of p.allowed_classes) admit({...valid,classification:c});
  for (const c of ['UNKNOWN','RESTRICTED','INTERNAL TEST ONLY']) assert.throws(() => admit({...valid,classification:c}));
  for (const k of p.required_fields) { const r={...valid}; delete r[k]; assert.throws(() => admit(r), { name: 'AssertionError' }); }
  assert.throws(() => admit({...valid,classification:'ODbL',derivation_review:'PENDING'}));
  assert.throws(() => admit({...valid,rights_review:'PENDING'}));
});

if (through >= 7) check('M007-architecture-decisions', () => {
...
if (through >= 8) check('M008-measurable-budgets', () => {
  ...
  assert.equal(Object.values(b.cost.monthly_allocations).reduce((a,v)=>a+v,0),b.cost.monthly_total_ceiling);
  assert.deepEqual(b.cost.actual_alert_percent,[50,80,100]);
  for (const key of ['invalid_published_geometry_max','broken_relationship_max','unsourced_published_claim_max','unreviewed_ai_claim_max']) assert.equal(b.limits[key],0);
