...
const normalize = (value) => {
  ...
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).filter(([k])=>k!=='path').sort(([a],[b])=>a.localeCompare(b)).map(([k,v])=>[k,normalize(v)]));
  return value;
};
...
for (const label of ['a','b']) {
  ...
  const graph=normalize(JSON.parse(run(process.execPath,[cli,'list','--recursive','--depth','Infinity','--json'],dir).stdout));
  ...
  assert.equal(run('git',['status','--porcelain'],dir).stdout.trim(),'','Install/smoke changed tracked source');
  installs.push({label,dir,lock_sha256:lockAfter,graph_sha256:hash(JSON.stringify(graph)),graph,install_exit:install.code});
...
const report={
  module:moduleId,result:'PASS',tested_commit:commit,node:process.version,
  ...
  graph_equal:true,stale_lock_rejected:true,unsupported_node_rejected:true,
  checked_at_utc:new Date().toISOString()
};
fs.writeFileSync(path.join(base,'report.json'),JSON.stringify(report,null,2)+'\n');
if(args.includes('--record')) fs.writeFileSync(path.join(root,'docs/progress',moduleId+'_INSTALL.json'),JSON.stringify(report,null,2)+'\n');
process.stdout.write('PASS identical actual graphs, unchanged lock, stale-lock and Node-engine rejection\n');
